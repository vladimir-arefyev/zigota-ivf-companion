# Zigota — Protocol Model (v3, evidence-grounded)
...
**What changed from v2 (alignment pass).** v2's §5 lifecycle predated the Phase 2 architecture and the Phase 3 plan, and had drifted from the vocabulary those documents settled. Four corrections, all in §5: (1) **`escalated` is no longer a state** — escalation is a safety *event* referencing an item, never a transition of it, because terminating a medication item when a patient reports a symptom is the opposite of safe; (2) **`confirmable` is removed** — it was a v2-only label with no counterpart in Requirements or Architecture, and `needs_selection` resolves directly to the confirmation gate; (3) **`missed` added** — the wake-anchored rollover state the requirements specify; (4) **`cancelled` added** — the terminal state cycle cancellation produces (Req 2.11). The item-type model, the three-phase arc, and the card traceability are unchanged: this is a vocabulary reconciliation, not a domain revision.
...
- **Model obligations:** requires its own **distinct, deliberate confirmation** (never the one-tap of a daily vitamin); on any reported timing problem (late/missed/wrong-time/spilled), it emits a `trigger_timing_problem` safety event surfacing the clinic-contact path, never a self-correction. Dose-timing correction is clinician-determined and out of scope.
...
The safety boundary is enforced by the state machine, not by prompt wording. This vocabulary is shared verbatim with `Zigota_Requirements` (glossary, lifecycle table) and `Zigota_Architecture` (§3.1); all three must agree, and this document is not the source of record when they disagree.

```
[parsed] --confirm--> [active] --patient confirms--> [done]
   |                     |
   |                     +--event time passes unconfirmed--> [missed]
   |                     |     (routine items: at the next wake anchor, not midnight)
   |                     |     (Type B trigger: the moment its exact time passes)
   |                     |
   |                     +--cycle ended by patient--------> [cancelled]
   |
   +-- (prescribed, no date) --> [prescribed_unscheduled] --date entered--> [active]
   +-- (has alternatives)   --> [needs_selection]         --patient picks--> [active]
   +-- (Type B trigger)     --> [needs_deliberate_confirm] --deliberate--> [active]
   +-- (missing field)      --> flagged incomplete         --completed---> [active]
```

`active` is the only mutable state. `done`, `missed` and `cancelled` are terminal against *automatic* change — no anchor edit re-times them — but `done` and `missed` stay patient-correctable, each correction recorded as an audit event.

**Escalation is an event, not a state.** v2 modelled `escalated` as a terminal state reached by "report-problem" from `active`. That is wrong, and concretely so: a patient reporting a D2 symptom on a day she still has doses to take would have had the referenced medication item terminated and its reminders suppressed. It also smuggled in an N2 violation, since a count of escalated items is a symptom-severity tally and the product is forbidden from grading symptoms.

The corrected model separates three record kinds:

- **schedule item lifecycle** — the states above. Governs scheduling and reminding, nothing else.
- **progress event** — what the patient did or reported: confirmation, symptom capture, check-in. Verbatim, timestamped.
- **safety event** — what the safety layer surfaced in response: `warning_match`, `contact_path_surfaced`, `instruction_unmet`, `trigger_timing_problem`. Carries a reference to the item or symptom report that occasioned it, and changes nothing about that item.

So a Type B timing problem, a Type E instruction reported unmet, and a Type D2 pattern match all produce safety events. The trigger stays whatever it was (`done` if confirmed late, `missed` if never confirmed); the Type E instruction item is unaffected; the medication schedule carries on. What the patient sees is the cited content and the clinic-contact path. What the record shows is that the app surfaced them.

Hard rules (each written to be **testable** — see the acceptance-criteria artifact):
...
- **A flagged-incomplete item cannot be confirmed** until the patient supplies the missing field; the parse never guesses it.
- **Type D2 matches emit a safety event with a verbatim clinic-redirect**, carry no assessment token, and leave every schedule item's lifecycle untouched.
- **A non-match is silent.** It emits no event and implies nothing reassuring; the clinic-contact path is available regardless of whether anything matched.
...
- No interpretation of symptoms, ultrasound findings, or lab/hormone values (a D2 pattern match surfaces cited content and the contact path without grading, and without altering any schedule item).
...
## 8. Open question carried into Phase 2 — *resolved in Requirements v0.3*

- **KB sourcing/validation stance** was the one genuine product-risk decision carried out of Phase 1. It is now settled, though not in this document: Requirements v0.3 Block 3 defines a **content taxonomy** (Safety / Clinical / General education) and partitions grounding accordingly. The curated KB holds Safety and Clinical content and answers those exclusively; an allowlisted retrieval index holds General education and is reached only after the KB returns nothing. The corpora do not overlap, which makes the split a safety property rather than a retrieval preference.
- What remains open is not sourcing but **validation**: KB entries carry a `review.validated_by` field that is null pending human clinical review. That is an honest-limits property the product surfaces rather than a blocker, and it is the natural hand-off point if a clinic later curates Tier 1 (the partition line and the clinic hand-off line are the same line).
